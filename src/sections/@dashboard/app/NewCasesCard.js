...
export default function NewCasesCard() {
 
  const NewCasesArray = useSelector(
  ...
  return (
    <RootStyle>
      <IconWrapperStyle>
        <Iconify icon="healthicons:virus-alt" width={24} height={24} />
      </IconWrapperStyle>
      {NewCasesArray.filter((item) => item.name === 'New Positive').map(
        (item) =>
          item.data
            .filter(
